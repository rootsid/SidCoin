...
#if defined(NDEBUG)
# error "SidCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd82673f3fc001cdbad384c0227ad98fa0f58099e13868aa7eae4627f5064c4af");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SidCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SidCoin Signed Message:\n";
...
{
    // SidCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // SidCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // SidCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // SidCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SidCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SidCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf1dea415cfa1ce9ee161c561b14aad827cda7954d2306bcfc7aa5136df49cfe1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SID Corp";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042d8e162d0964ab9d87c1022cf520b813ec9355552503232cf605c0b1db496b5fa349ed1ca1e0a56e832eb16c42a8b2f19f7a7608c214551542256188a29c45d8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522300517;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084708108;

        if (fTestNet)
        {
            block.nTime    = 1522300484;
            block.nNonce   = 385319112;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x72ddb3e51cce89ea89adc30ffe5cdd74036811745317161f8c4ad4a3957acb28"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc7, 0xb8, 0xd6 }; // SidCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SidCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SidCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SidCoinMiner(CWallet *pwallet)
{
    printf("SidCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sidcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SidCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SidCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SidCoinMiner, pwallet));
}
