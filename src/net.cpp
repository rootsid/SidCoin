...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SidCoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x0100007f, 0x30b3ce67, 0x6700a8c0
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SidCoin is probably already running."), addrBind.ToString().c_str());
        else
